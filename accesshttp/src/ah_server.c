...
static int init_methods_handler(void)
{
    ...
    ERR_RET_LN(add_handler("order.finished_all", readhistory, CMD_ORDER_HISTORY_ALL));
    ...
    ERR_RET_LN(add_handler("market.add_asset", matchengine, CMD_MARKET_ADD_ASSET));
    ERR_RET_LN(add_handler("market.del_asset", matchengine, CMD_MARKET_DEL_ASSET));
    ERR_RET_LN(add_handler("market.add_market", matchengine, CMD_MARKET_ADD_MARKET));
