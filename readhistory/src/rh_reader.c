...
{
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "SELECT `id`, `time`, `asset`, `business`, `change`, `balance`, `detail` FROM `balance_history_%u` WHERE `user_id` = %u",
    ...
    for (size_t i = 0; i < num_rows; ++i)
    {
        ...
        uint64_t balance_id = strtoull(row[0], NULL, 0);
        json_object_set_new(record, "id", json_integer(balance_id));
        json_object_set_new(record, "time", json_real(timestamp));
        json_object_set_new(record, "asset", json_string(row[2]));
        json_object_set_new(record, "business", json_string(row[3]));
        json_object_set_new(record, "change", json_string(rstripzero(row[4])));
        json_object_set_new(record, "balance", json_string(rstripzero(row[5])));
        json_t *detail = json_loads(row[6], 0, NULL);
...
json_t *get_user_order_finished_all(MYSQL *conn, uint32_t user_id,
...
json_t *get_market_user_deals(MYSQL *conn, uint32_t user_id, const char *market, size_t offset, size_t limit)
{
    ...
    for (size_t i = 0; i < num_rows; ++i)
    {
        ...
        json_object_set_new(record, "market", json_string(rstripzero(row[10])));
