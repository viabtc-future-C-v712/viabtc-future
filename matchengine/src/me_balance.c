...
int add_balance(const char* name, int prec_save, int prec_show)
{
    if (!name)
        return -1;

    struct asset_type *at = get_asset_type(name);
    if (at) {
        at->prec_save = prec_save;
        at->prec_show = prec_show;
    } else {
        struct asset_type type;
        type.prec_save = prec_save;
        type.prec_show = prec_show;
        if (dict_add(dict_asset, (void*)name, &type) == NULL)
            return -1;
    }
...
int del_balance(const char* name)
{
    if (!name)
        return -1;
    ...
    while ((entry = dict_next(iter)) != NULL) {
        key = entry->key;
        if (strcmp(name, key->asset) == 0)
            dict_delete(dict_balance, entry->key);
    }
    dict_release_iterator(iter);

    // delete asset
    dict_delete(dict_asset, name);
