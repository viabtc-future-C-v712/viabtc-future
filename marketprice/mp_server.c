...
static int on_cmd_market_status(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    if (period <= 0 || period > 86400/*settings.sec_max*/)
        return reply_error_invalid_argument(ses, pkg);
...
static int on_cmd_market_kline_reload(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    const char* rc = market_kline_reload(market);

    json_t *result = json_object();
    json_object_set_new(result, "message", json_string(rc));
...
static int on_cmd_market_kline_update(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    const char *k_type = json_string_value(json_array_get(params, 1));
    if (!k_type)
        return reply_error_invalid_argument(ses, pkg);

    if (!json_is_integer(json_array_get(params, 2)))
        return reply_error_invalid_argument(ses, pkg);
    time_t tm = json_integer_value(json_array_get(params, 2));

    const char *open = json_string_value(json_array_get(params, 3));
    if (!open)
        return reply_error_invalid_argument(ses, pkg);

    const char *close = json_string_value(json_array_get(params, 4));
    if (!close)
        return reply_error_invalid_argument(ses, pkg);

    const char *high = json_string_value(json_array_get(params, 5));
    if (!high)
        return reply_error_invalid_argument(ses, pkg);

    const char *low = json_string_value(json_array_get(params, 6));
    if (!low)
        return reply_error_invalid_argument(ses, pkg);

    const char *volume = json_string_value(json_array_get(params, 7));
    if (!volume)
        return reply_error_invalid_argument(ses, pkg);

    const char *deal = json_string_value(json_array_get(params, 8));
    if (!deal)
        return reply_error_invalid_argument(ses, pkg);

    const char* rc = market_kline_update(market, k_type, tm, open, close, high, low, volume, deal);
...
static void svr_on_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_MARKET_STATUS_TODAY:
        ...
        break;
    case CMD_MARKET_KLINE_RELOAD:
        log_debug("from: %s cmd market kline reload, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
        ret = on_cmd_market_kline_reload(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_market_kline_reload %s fail: %d", params_str, ret);
        }
        break;
    case CMD_MARKET_KLINE_UPDATE:
        log_debug("from: %s cmd market kline update, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
        ret = on_cmd_market_kline_update(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_market_kline_update %s fail: %d", params_str, ret);
        }
        break;
    default:
