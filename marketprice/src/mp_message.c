...
int init_message(void)
{
    ...
    nw_timer_set(&redis_timer, 3600, true, on_redis_timer, NULL);
    nw_timer_start(&redis_timer);

    return 0;
}

const char* market_kline_reload(const char* market)
{
    time_t now = time(NULL);
    ...
    if (!info)
        ...
        clear_dict(info->sec, now);
        clear_dict(info->min, now);
        clear_dict(info->hour, now);
        clear_dict(info->day, now);
        clear_dict(info->update, now);
        list_clear(info->deals);
        list_clear(info->deals_json);
    }

    redisContext *context = redis_sentinel_connect_master(redis);
    if (context != NULL) {
        int rc = load_market(context, info);
        if (rc < 0) {
            log_error("load market %s fail: %d", market, rc);
            ret = "load market failed";
        }
        redisFree(context);
    } else {
        ret = "connect redis failed";
    }

    return ret;
}

const char* market_kline_update(const char* market, const char* k_type, time_t tm, const char* open, const char* close, const char* high, const char* low, const char* volume, const char* deal)
{
    const char* ret = "success";
    struct market_info *info = market_query(market);
    if (!info)
    ...
    dict_t* dt = NULL;
    if (strcmp(k_type, "sec") == 0)
        dt = info->sec;
    else if (strcmp(k_type, "min") == 0)
        dt = info->min;
    else if (strcmp(k_type, "hour") == 0)
        dt = info->hour;
    else if (strcmp(k_type, "day") == 0)
        dt = info->day;
    else
        return "kline type not support";

    struct kline_info *k_info = kline_query(dt, tm);
    if (!k_info)
        return "kline not found for the timestamp";

    mpd_t* mpd_open = decimal(open, 0);
    mpd_t* mpd_close = decimal(close, 0);
    mpd_t* mpd_high = decimal(high, 0);
    mpd_t* mpd_low = decimal(low, 0);
    mpd_t* mpd_volume = decimal(volume, 0);
    mpd_t* mpd_deal = decimal(deal, 0);
    if (!mpd_open || !mpd_close || !mpd_high || !mpd_low || !mpd_volume || !mpd_deal) {
        ret = "kline information not available";
        goto cleanup;
    }

    mpd_copy(k_info->open, mpd_open, &mpd_ctx);
    mpd_copy(k_info->close, mpd_close, &mpd_ctx);
    mpd_copy(k_info->high, mpd_high, &mpd_ctx);
    mpd_copy(k_info->low, mpd_low, &mpd_ctx);
    mpd_copy(k_info->volume, mpd_volume, &mpd_ctx);
    mpd_copy(k_info->deal, mpd_deal, &mpd_ctx);
