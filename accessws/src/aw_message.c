...
static kafka_consumer_t *kafka_positions;
...
static int process_positions_message(json_t *msg)
{
    uint32_t user_id = json_integer_value(json_array_get(msg, 1));
    const char *market = json_string_value(json_array_get(msg, 2));
    uint32_t side = json_integer_value(json_array_get(msg, 3));
    if (user_id == 0 || market == NULL || side == 0) {
        return -__LINE__;
    }

    position_on_update(user_id, market, side);
...
static void on_positions_message(sds message, int64_t offset)
{
    log_trace("position message: %s", message);
    json_t *msg = json_loads(message, 0, NULL);
    if (!msg) {
        log_error("invalid position message: %s", message);
        return;
    }

    int ret = process_positions_message(msg);
    if (ret < 0) {
        log_error("process_positions_message: %s fail: %d", message, ret);
    }
...
int init_message(void)
{
    ...
    settings.positions.offset = RD_KAFKA_OFFSET_END;
    kafka_positions = kafka_consumer_create(&settings.positions, on_positions_message);
    if (kafka_positions == NULL) {
        return -__LINE__;
    }
