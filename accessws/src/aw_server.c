...
static int send_json(nw_ses *ses, const json_t *json)
{
    ...
    log_trace("send to: %"PRIu64", size: %zu, message: %s ret:%d", ses->id, strlen(message_data), message_data, ret);
    free(message_data);
    return ret;
}
...
static int on_method_order_query_all(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    if (!rpc_clt_connected(matchengine))
        return send_error_internal_error(ses, id);
...
static int on_method_order_history_all(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
...
static int on_method_order_subscribe(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    ...
    size_t params_size = json_array_size(params);
    if (params_size == 0) {
        if (order_subscribe(info->user_id, ses, NULL) < 0)
            return send_error_internal_error(ses, id);
    } else {
        for (size_t i = 0; i < params_size; ++i) {
            const char *market = json_string_value(json_array_get(params, i));
...
static int init_svr(void)
{
    ...
    ERR_RET_LN(add_handler("order.query_all",   on_method_order_query_all));
    ERR_RET_LN(add_handler("order.history",     on_method_order_history));
    ERR_RET_LN(add_handler("order.history_all", on_method_order_history_all));
